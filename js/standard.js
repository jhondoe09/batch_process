...
function save_third_tbl_data(operator_number) {
    ...
    for (let j = 0; j < third_tbl_tr.length; j++) {
        ...
        const third_tbl_ng_reason = third_tbl_td_element[9].querySelector('select').value;
...
function get_table_data(operator_number, total_batch_processed) {
    ...
    accordion.addEventListener('shown.bs.collapse', function (e) {
                    ...
                    if (localStorage.getItem('operator_status') == 'Started') {
                        ...
                        Swal.fire({
                            ...
                            if (result.dismiss === Swal.DismissReason.timer) {
                                swalALert("Data Fetched", "Done! Thanks for your patient ;)", "success");
                            }
                        })
                    }
                    for (let data of second_table_datas) {
                        ...
                        const tr =
                            `<tr id="second_table_tr_${data.operator_number}_${data.line_number}">
                                ...
                                <td><input id="wafer_no_from_${data.operator_number}_${data.line_number}" type="number" class="form-control bg-light" value="${data.wafer_number_from ? data.wafer_number_from : data.waferFrom ? data.waferFrom : ''}" style="width:${data.waferFrom.length + 4 ? data.waferFrom.length + 4 : data.wafer_number_from.length + 4}ch"></td>
                                <td><input id="wafer_no_to_${data.operator_number}_${data.line_number}" type="number" class="form-control bg-light" onkeydown="onkeydown_wafer_no_to(${data.operator_number}, ${data.line_number})" value="${data.wafer_number_to ? data.wafer_number_to : data.waferTo ? data.waferTo : ''}" style="width:${data.waferTo.length + 5 ? data.waferTo.length + 5 : data.wafer_number_to.length + 5}ch"></td>
        ...
        // get_thirdtable_data.append('line_number', localStorage.getItem('line_number'));
        // get_thirdtable_data.append('parts_number', localStorage.getItem('parts_number'));
        // get_thirdtable_data.append('revision_number', localStorage.getItem('revision_number'));
        // get_thirdtable_data.append('lot_number', localStorage.getItem('lot_number'));
            ...
            if (qty_data.success) {
                for (let data of qty_data.data) {
                    if (data.quantity_out) {
                        document.getElementById(`qty_in_${operator_number}_${line_number}`).value = data.quantity_out;
                    }
                }
            }
        })
        ...
        .then(NGWaferData => {
            ...
            if (NGWaferData) {
                if (NGWaferData.success) {
                    ...
                    const wafer_from2 = document.getElementById(`wafer_no_from_${operator_number}_${NGWaferData.data[0].line_number}}`);
                    ...
                    const total = parseInt(wafer_to2.value) - parseInt(wafer_from2.value);
